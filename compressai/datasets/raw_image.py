...
class RawImageDataset(Dataset):
    ...
    def __init__(self, root, transform=None, split="train", in_memory=False, ntest=16, ntrain=-1, repeat=1, **kwargs):
        ...
        self.raw_space = kwargs.pop("raw_space", "raw_linear")
        ...
        with open(self.file_path, "r") as f:
            lines = f.readlines()
        print(f"[INFO] Using color space [{self.raw_space}]")
        # if self.demosaic:
    ...
    def __getitem__(self, index):
        ...
        if format_raw.lower() in ["png",]:
            raw_img = cv2.imread(raw_path, cv2.IMREAD_UNCHANGED)
            white_balance = -1
            raw_visible = raw_img
        elif format_raw.lower() in ["tif", "tiff"]:
            raw_img = skimage.io.imread(raw_path,plugin='tifffile')
            white_balance = -1
            raw_visible = raw_img
        else: # raw file
            ...
            if self.demosaic:
                ...
                elif self.raw_space == "raw_gamma":
                    raw_visible = raw_img.postprocess(output_color=rawpy.ColorSpace.raw, use_camera_wb=True, use_auto_wb=False, no_auto_bright=True, output_bps=16) 
                elif self.raw_space == "raw_linear":
                    raw_visible = raw_img.postprocess(output_color=rawpy.ColorSpace.raw, use_camera_wb=True, use_auto_wb=False, no_auto_bright=True, output_bps=16, gamma=(1,1))
                elif self.raw_space == "cvpr2022":
                    raw_visible = raw_img.postprocess(output_color=rawpy.ColorSpace.raw, use_camera_wb=False, use_auto_wb=False, no_auto_bright=True, output_bps=16, gamma=(1,1), user_wb=[1,1,1,1]) # no_auto_scale=True
                elif self.raw_space == "sid":
                    raw_visible = self.pack_raw(raw_img)
                    raw_visible = (raw_visible / raw_visible.mean()) * 65535.
        ...
        jpg_bpp = 24          
        if len(self.samples[index]) == 1:
            if self.raw_space == "sid":
                srgb_image = raw_img.postprocess(use_camera_wb=True, half_size=True, no_auto_bright=False)
            else:
            ...
            if format_rgb.lower() in ["jpg", "jpeg", "tif", "png"]:
        ...
        # if self.raw_space == "cvpr22":
        #     img_list = [F.interplote(srgb_image,scale_factor=1/4), F.interplote(raw_visible,scale_factor=1/4)]
